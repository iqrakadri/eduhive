...
export default function LoginPage() {
  ...
  const router = useRouter(); // Keeping this just for the back button, but using window.location for auth
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 2. Check for completed profile based on the single-role policy
      ...
      if (profileDoc.exists()) {
        ...
        // HARD REDIRECT to bypass Next.js router conflicts
        window.location.href = `/dashboard/${role}`;
        
      } else {
        // --- CASE B: SIGNED UP BUT NO PROFILE YET ---
        toast.loading("Completing setup...");
        
        // HARD REDIRECT to registration
        window.location.href = `/register/${role}?uid=${user.uid}&email=${user.email}`;
      }
